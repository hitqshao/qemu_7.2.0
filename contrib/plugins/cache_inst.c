...
static uint64_t instCnt;
static uint64_t instBCnt;
...
static uint64_t pre_l1_imisses;
static uint64_t pre_l1_dmisses;
...
static void vcpu_insn_exec(unsigned int vcpu_index, void *userdata)
{
    ...
    // output cacheinfo every 1B
    instCnt++;
    if (instCnt % 1000000000 == 0) {
        instBCnt+=1;
        instCnt = 0;
        g_autoptr(GString) rep = g_string_new("");
        g_string_append_printf(rep, "%ld",instBCnt);
        g_string_append(rep, "000000000 insts executed.\n");
        qemu_plugin_outs(rep->str);
        log_stats();

        uint64_t cur_l1_imisses = l1_icaches[cache_idx]->misses - pre_l1_imisses;
        uint64_t cur_l1_dmisses = l1_dcaches[cache_idx]->misses - pre_l1_dmisses;

        pre_l1_imisses = l1_icaches[cache_idx]->misses;
        pre_l1_dmisses = l1_dcaches[cache_idx]->misses;

        g_autoptr(GString) missrep = g_string_new("");
        g_string_append_printf(missrep, "%ld",cur_l1_imisses/1000000);
        g_string_append(missrep, " icache misses | ");
        g_string_append_printf(missrep, "%ld",cur_l1_dmisses/1000000);
        g_string_append(missrep, " dcache misses .\n");
        qemu_plugin_outs(missrep->str);
    }
